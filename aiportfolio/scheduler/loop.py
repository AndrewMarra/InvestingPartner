...
from ..storage.db import Store, _now
...
class Engine:
    def __init__(self, config_path=None, config=None, user_id="local"):
        ...
        self.store = Store(self.cfg["storage"]["db_path"], user_id=user_id)
        self.notifier = Notifier(s, self.cfg)
        self.lead = int((self.cfg.get("trading", {}) or {}).get("notify_lead_minutes", 10))
    ...
    def _option_quoter(self, u, r, k, z):
        if not self.options: return None
        c = self.options.find_contract(u, r, k, z)
    ...
    def run_cycle(self, force=False, skip_triage=False):
        ...
        # 2) due planned exits -> notify + queue a sell
        ...
        day_dd = (account["equity"] / day_open - 1) * 100 if day_open else 0
        ...
        for t in approved:
            ...
            console.print(f"[bold green]QUEUED[/] [{t.mode}] {label} "
